...
async fn update_waypoint(
    ...
    for n in swarm.validators_mut() {
        ...
        update_node_config(n, node_config)?;
    }
    Ok(())
}
...
pub async fn make_twin_swarm(
    ...
    for n in smoke.swarm.validators() {
        let cred = extract_credentials(n).await?;
        creds.push(cred);
    }
    ...
    let temp_db_path = temp_backup_db(&reference_db, temp_path)?;
    dbg!(&temp_db_path);
    assert!(temp_db_path.exists());

    println!("2. Create a rescue blob from the reference db");

    let rescue_blob_path = make_rescue_twin_blob(&temp_db_path, creds).await?;

    println!("3. Apply the rescue blob to the swarm db & bootstrap");

    let wp = apply_rescue_on_db(&temp_db_path, &rescue_blob_path)?;

    println!("4. Replace the swarm db with the snapshot db");

    replace_db_all(&mut smoke.swarm, &temp_db_path).await?;

    println!("5. Change the waypoint in the node configs and add the rescue blob to the config");
    update_waypoint(&mut smoke.swarm, wp, rescue_blob_path).await?;

    println!("6. wait for liveness");
    restart_all(&mut smoke.swarm)?;
...
async fn test_setup_twin_with_noop_db() -> anyhow::Result<()> {
    ...
    let version_old = smoke
        ...
        .inner()
        .version;
    make_twin_swarm(&mut smoke, None, false).await?;
